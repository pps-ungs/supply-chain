        ...
        for i in range(len(num_iterations)):
            for s in range(len(num_step)):
                for n in range(len(num_neigbors)):
                    result = neighborhood.run_heuristic_with_neighbors_strategy(
                        model=model, step=num_step[s], neighbor_strategy=func,
                        num_neighbors=num_neigbors[n], max_iterations=num_iterations[i]
                    )
                    results.append(result)
        writeCSV(filename=f"{dir}/results_neighbors_{name}.csv", rows=results)

def run_eval_neighbors_experiment(dir:str, model):
    ...
    for name, func in eval_strategies.items():
        ...
        for i in range(len(num_iterations)):
            for s in range(len(num_step)):
                result = neighborhood.run_heuristic_with_eval_strategy(
                    model=model, step=num_step[s], eval_strategy=func,
                    max_iterations=num_iterations[i]
                )
                results.append(result)
        writeCSV(filename=f"{dir}/results_neighbors_{name}.csv", rows=results)

def run_creation_eval_neighbors_experiment(dir:str, model):
            ...
            for i in range(len(num_iterations)):
                for s in range(len(num_step)):
                    if name_c != "exhaustive":
                        for n in range(len(num_neigbors)):
                            result = neighborhood.run_heuristic_with_all_strategies(
                                model=model, step=num_step[s], neighbor_strategy=func_c,
                                eval_strategy=func_e, num_neighbors=num_neigbors[n],
                                max_iterations=num_iterations[i]
                            )
                            results.append(result)
                    else:
                        result = neighborhood.run_heuristic_with_all_strategies(
                            model=model, step=num_step[s], neighbor_strategy=func_c,
                            eval_strategy=func_e, num_neighbors=0,
                            max_iterations=num_iterations[i]
                        )
                        results.append(result)
